...
import bson
from mongokit import ReplicaSetConnection, Connection, Database, Collection, Document
...
class MongoKit(object):
    ...
    def connect(self):
        ...
        if mongokit_connection is None:
            if ctx.app.config.get('MONGODB_REPLSET_URL'):
                ctx.mongokit_connection = ReplicaSetConnection(
                    hosts_or_uri=ctx.app.config.get('MONGODB_REPLSET_URL'),
